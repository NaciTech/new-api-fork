...
func (channel *Channel) HasEnabledKey() bool {
	return channel.HasEnabledKeyExcluding(nil)
}

func (channel *Channel) HasEnabledKeyExcluding(excluded map[int]bool) bool {
	if channel == nil {
		return false
	}
	if !channel.ChannelInfo.IsMultiKey {
...
func (channel *Channel) GetEnabledKeyByIndex(index int) (string, int, string, bool) {
	if channel == nil {
		return "", 0, "channel_nil", false
	}
	if !channel.ChannelInfo.IsMultiKey {
		return "", 0, "not_multi_key", false
	}
	keys := channel.GetKeys()
	if len(keys) == 0 {
		return "", 0, "no_keys", false
	}
	...
	if index < 0 || index >= len(keys) {
		return "", 0, "preferred_key_out_of_range", false
	}
	...
	if status != common.ChannelStatusEnabled {
		return "", 0, "preferred_key_disabled", false
	}
	key := strings.TrimSpace(keys[index])
	if key == "" {
		return "", 0, "preferred_key_empty", false
	}
